...
Storage::Storage(size_t num_bytes, Device device)
    : _buffer(new std::byte[num_bytes]), _refcount(nullptr), _size(num_bytes),
      _device(device) {}
...
size_t Storage::use_count() const { return 1; } // STUB: not real refcounting

Storage::~Storage() {} // STUB: leaks intentionally

// Copy SHARES the buffer (shallow) so a Tensor's views point at the same bytes.
Storage::Storage(const Storage &other)
    : _buffer(other._buffer), _refcount(nullptr), _size(other._size),
